# 12.多模态

在英语学习过程中，口语和听力也是重要的一部分，否则就成了哑巴英语。为了 贴近真实语言使用场景，本章节将让那个英语学习 Agent 支持「语音输入 + 语音输出」的多模态能力


## 一、架构流程调整

整个流程将调整如下：

```
🎙️ User speaks
   ↓
🧩 Speech-to-Text (ASR)
   ↓
📜 Chat History + Prompt
   ↓
🧠 LLM (English Tutor Agent)
   ↓
📄 Text Response
   ↓
🔊 Text-to-Speech (TTS)
   ↓
🎧 Audio Playback
```

## 二、前端调整

在Day09中讲到Gradio支持Interface (快速模式)和Blocks (自定义模式)，前面用的是快速模式，现在已经不够用了。这里要切换为自定义模式。

1. Gradio 中的语音输入

Gradio 原生支持麦克风输入：

```python
gr.Audio(
    sources=["microphone"],
    type="filepath",
    label="Speak English"
)
```

这里拿到的是：

```text
/tmp/gradio/xxx.wav
```

2. ASR 示例

基于openai的whisper

```python
import whisper
asr_model = whisper.load_model("turbo")

def speech_to_text(audio_path: str) -> str:
    """
    把用户语音转成文本
    """
    text = asr_model.transcribe(audio_path)
    return text
```

3. 把语音输入接入现有对话 Agent

修改原来的获取回复部分代码，将以下代码：

```python
get_ai_response(user_message: str, history)
```

改成：

```python
def voice_chat(audio_path, history):
    user_text = speech_to_text(audio_path)
    return get_ai_response(user_text, history)
```


4. 语音输出：Text-to-Speech（TTS）

```python

import edge_tts

def text_to_speech(text: str) -> str:
    """
    输入文本 → 输出音频文件路径
    """
    audio_path = "./output.mp3"
    communicate = edge_tts.Communicate(text, "en-GB-SoniaNeural")
    with open(audio_path, "wb") as file:
        for chunk in communicate.stream_sync():
            if chunk["type"] == "audio":
                file.write(chunk["data"])
    return audio_path
```

5. 把 Agent 回复转成语音
基于tts-edge实现

```python
response_text = get_ai_response(user_text, history)
audio_path = text_to_speech(response_text)

return response_text, audio_path
```

6. 调整UI

基于Gradio的自定义模式实现，代码如下：

```python
with gr.Blocks() as demo:
    gr.Markdown("## 🎧 英语口语学习助手")

    audio_input = gr.Audio(
        sources=["microphone"],
        type="filepath",
        label="Speak English"
    )

    text_output = gr.Chatbot()
    audio_output = gr.Audio(label="AI Voice Reply")

    audio_input.change(
        fn=voice_chat,
        inputs=[audio_input, text_output],
        outputs=[text_output, audio_output]
    )
```

至此，英语学习Agent支持语音输入输出了。完整代码参考`12_code`文件夹。